...
async function verifyMember(member, ign, edition, device, region, staffOverride = false) {
    ...
    // Try to change nickname with better error handling
    let nicknameChanged = false;
    try {
        await member.setNickname(ign);
        nicknameChanged = true;
        console.log(`✅ Nickname changed to ${ign} for ${member.user.tag}`);
    } catch (error) {
        console.error(`❌ Failed to change nickname for ${member.user.tag}: ${error.message}`);
    ...
    let message = `✅ Verified as **${ign}**! You now have access to all channels.`;
    if (!nicknameChanged) {
        message += `\n\n⚠️ **Could not change your nickname.**\nPlease ask a staff member to move the bot's role above your roles in Server Settings → Roles.`;
    }
...
client.once('ready', async () => {
    ...
    console.log('');
    console.log('⚠️ IMPORTANT: For nickname changes to work, make sure the bot role is ABOVE member roles in:');
    console.log('   Server Settings → Roles → Drag IGN Verifier role to the top (below @everyone)');
